 ...
 *
 *  This program is a simple implementation of encryption using 
 *  one-time pad-like system utilizing sockets for inter-process
 *  communcation and multiple processes. This file is used in
 *  conjunction with otp_enc.c to send text.
 */
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
void encrypt(char *plainText, char *key, long length);
     ...
     while(1){
    	 ...
    	 case 0:
		  ...
		  //   	perror("invalid client");
		     ...
		     //Encrypt
			 encrypt(text, key, fileLength);
		 	 
			 //Send encryption back
...
/* Description: encodes text from key
 * Parameters: c-string of text to be encoded, c-string of key,
 * and long of c-string length
 * Post-condition: encoded text based on key; results will be in
 * text itself
 */
void encrypt(char *text, char *key, long length){
	char x,y;
	int i;
	for(i = 0; i < length-1; i++){
		x = text[i];
		y = (char_to_int(text[i]) + char_to_int(key[i])) % 27;
		text[i] = int_to_char(y);	
	}
}
